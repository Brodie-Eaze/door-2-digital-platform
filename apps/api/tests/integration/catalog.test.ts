...
async function seed(): Promise<void> {
  await prisma().org.createMany({
    data: [
      { id: orgA, legalName: 'Cat A', tradingName: 'CA', vertical: 'charity', type: 'client', regionCode: 'US' },
      { id: orgB, legalName: 'Cat B', tradingName: 'CB', vertical: 'commercial', type: 'client', regionCode: 'US' },
    ],
  });
  await prisma().user.createMany({
    data: [
      { id: adminA, orgId: orgA, email: adminEmailA, emailDigest: emailDigest(adminEmailA, process.env.PII_SEARCH_KEY!), givenName: 'A', familyName: 'Admin', role: 'org_admin', regionCode: 'US' },
      { id: managerA, orgId: orgA, email: managerEmailA, emailDigest: emailDigest(managerEmailA, process.env.PII_SEARCH_KEY!), givenName: 'M', familyName: 'Manager', role: 'manager', regionCode: 'US' },
      { id: knockerA, orgId: orgA, email: knockerEmailA, emailDigest: emailDigest(knockerEmailA, process.env.PII_SEARCH_KEY!), givenName: 'K', familyName: 'Nocker', role: 'knocker', regionCode: 'US' },
      { id: adminB, orgId: orgB, email: adminEmailB, emailDigest: emailDigest(adminEmailB, process.env.PII_SEARCH_KEY!), givenName: 'B', familyName: 'Admin', role: 'org_admin', regionCode: 'US' },
    ],
  });
...
describe('GET /v1/catalog', () => {
  it('returns active offerings ordered by sortOrder asc, only the iOS fields', async () => {
    ...
    await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-seed-2' }, payload: { ...validOffering, name: 'Second', sortOrder: 2 } });
    await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-seed-1' }, payload: { ...validOffering, name: 'First', sortOrder: 1, highlighted: true } });

    const knockerToken = await tokenFor(knockerEmailA, knockerPassA);
    const res = await app.inject({ method: 'GET', url: '/v1/catalog', headers: { authorization: `Bearer ${knockerToken}` } });
    ...
    expect(Object.keys(first).sort()).toEqual(
      ['amountCents', 'blurb', 'frequency', 'highlighted', 'id', 'name', 'sortOrder', 'vertical'].sort(),
    );
  ...
  it('excludes soft-deleted offerings', async () => {
    ...
    const created = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-del-1' }, payload: validOffering });
    const id = created.json().id;
    await app.inject({ method: 'DELETE', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${t}` } });
    const res = await app.inject({ method: 'GET', url: '/v1/catalog', headers: { authorization: `Bearer ${t}` } });
    expect(res.json()).toHaveLength(0);
  });

  it('is tenant-scoped — org B never sees org A offerings', async () => {
    const tA = await tokenFor(adminEmailA, adminPassA);
    await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${tA}`, 'idempotency-key': 'cat-iso-1' }, payload: validOffering });
    const tB = await tokenFor(adminEmailB, adminPassB);
    const res = await app.inject({ method: 'GET', url: '/v1/catalog', headers: { authorization: `Bearer ${tB}` } });
    expect(res.json()).toHaveLength(0);
  });
...
describe('POST /v1/catalog', () => {
  it('manager can create — returns 201 + id, writes audit row', async () => {
    const t = await tokenFor(managerEmailA, managerPassA);
    const res = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-create-1' }, payload: validOffering });
    expect(res.statusCode).toBe(201);
    expect(res.json().id).toMatch(/^svo_/);
    const audit = await prisma().auditEvent.findFirst({ where: { orgId: orgA, action: 'service_offering.created' } });
    expect(audit).toBeTruthy();
  });

  it('knocker is rejected (403)', async () => {
    const t = await tokenFor(knockerEmailA, knockerPassA);
    const res = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-rbac-1' }, payload: validOffering });
    expect(res.statusCode).toBe(403);
  });

  it('idempotency replay returns the same id', async () => {
    const t = await tokenFor(adminEmailA, adminPassA);
    const first = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-replay-1' }, payload: validOffering });
    const second = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-replay-1' }, payload: validOffering });
  ...
  it('Zod rejects amountCents <= 0', async () => {
    const t = await tokenFor(adminEmailA, adminPassA);
    const res = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-bad-amt-1' }, payload: { ...validOffering, amountCents: 0 } });
    expect(res.statusCode).toBe(400);
  });

  it('Zod rejects unknown vertical and unknown keys', async () => {
    const t = await tokenFor(adminEmailA, adminPassA);
    const badVertical = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-bad-vert-1' }, payload: { ...validOffering, vertical: 'crypto' } });
    expect(badVertical.statusCode).toBe(400);
    const extraKey = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-bad-key-1' }, payload: { ...validOffering, orgId: orgB } });
    expect(extraKey.statusCode).toBe(400);
  });

  it('Zod rejects empty name and oversized name', async () => {
    const t = await tokenFor(adminEmailA, adminPassA);
    const empty = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-empty-1' }, payload: { ...validOffering, name: '' } });
    expect(empty.statusCode).toBe(400);
    const big = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-big-1' }, payload: { ...validOffering, name: 'x'.repeat(201) } });
...
describe('PATCH /v1/catalog/:id', () => {
  async function createOne(): Promise<{ id: string; token: string }> {
    ...
    const created = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${token}`, 'idempotency-key': `cat-patch-seed-${Math.random()}` }, payload: validOffering });
  ...
  it('manager can update fields and the change is reflected', async () => {
    ...
    const res = await app.inject({ method: 'PATCH', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${token}` }, payload: { name: 'Hope Premium', amountCents: 6000 } });
    expect(res.statusCode).toBe(200);
    expect(res.json().id).toBe(id);
    const get = await app.inject({ method: 'GET', url: '/v1/catalog', headers: { authorization: `Bearer ${token}` } });
    expect(get.json()[0].name).toBe('Hope Premium');
    expect(get.json()[0].amountCents).toBe(6000);
    const audit = await prisma().auditEvent.findFirst({ where: { orgId: orgA, action: 'service_offering.updated' } });
    expect(audit).toBeTruthy();
  });
  ...
  it('knocker is rejected (403)', async () => {
    ...
    const t = await tokenFor(knockerEmailA, knockerPassA);
    const res = await app.inject({ method: 'PATCH', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${t}` }, payload: { name: 'x' } });
    expect(res.statusCode).toBe(403);
  });

  it('unknown id 404s', async () => {
    const t = await tokenFor(adminEmailA, adminPassA);
    const res = await app.inject({ method: 'PATCH', url: '/v1/catalog/svo_does_not_exist', headers: { authorization: `Bearer ${t}` }, payload: { name: 'x' } });
  ...
  it("cross-tenant id 404s (org B can't patch org A)", async () => {
    const { id } = await createOne();
    const tB = await tokenFor(adminEmailB, adminPassB);
    const res = await app.inject({ method: 'PATCH', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${tB}` }, payload: { name: 'pwned' } });
    expect(res.statusCode).toBe(404);
  });
});
...
describe('DELETE /v1/catalog/:id', () => {
  it('soft-deletes (active=false), never hard-deletes', async () => {
    const t = await tokenFor(managerEmailA, managerPassA);
    const created = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-soft-1' }, payload: validOffering });
    const id = created.json().id;
    const res = await app.inject({ method: 'DELETE', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${t}` } });
    expect(res.statusCode).toBe(200);
    expect(res.json().id).toBe(id);
    const row = await prisma().serviceOffering.findUnique({ where: { id } });
    expect(row).toBeTruthy();
    expect(row?.active).toBe(false);
    const audit = await prisma().auditEvent.findFirst({ where: { orgId: orgA, action: 'service_offering.archived' } });
    expect(audit).toBeTruthy();
  });

  it('knocker is rejected (403)', async () => {
    const t = await tokenFor(managerEmailA, managerPassA);
    const created = await app.inject({ method: 'POST', url: '/v1/catalog', headers: { authorization: `Bearer ${t}`, 'idempotency-key': 'cat-soft-rbac-1' }, payload: validOffering });
    const id = created.json().id;
    const kt = await tokenFor(knockerEmailA, knockerPassA);
    const res = await app.inject({ method: 'DELETE', url: `/v1/catalog/${id}`, headers: { authorization: `Bearer ${kt}` } });
    expect(res.statusCode).toBe(403);
  });
});
