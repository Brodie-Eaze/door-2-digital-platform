...
/**
 * There is no Stripe webhook-event log, payment-mandate table, or cohort
 * retention aggregate in the schema — the fixtures that used to live here
 * (STRIPE_EVENTS, MANDATES, COHORTS) were pure fabrication. What IS real:
 * Conversion.amountCents/d2dRakeCents/processorResidualCents and
 * Donation.status, aggregated region-wide by GET /api/regions/AU/billing.
 */
...
export default function AuPaymentsPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const res = await fetch('/api/regions/AU/billing', {
      ...
      <div className="space-y-5 max-w-[1700px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard
            ...
            value={<Money cents={BigInt(billing?.mtd.volumeCents ?? '0')} region="AU" />}
            hint={`${billing?.mtd.conversionCount ?? 0} conversions`}
          />
          <KpiCard
            label="D2D rake MTD"
            value={<Money cents={BigInt(billing?.mtd.rakeCents ?? '0')} region="AU" />}
          />
          <KpiCard
            label="Processor residual MTD"
            value={<Money cents={BigInt(billing?.mtd.residualCents ?? '0')} region="AU" />}
          />
          <KpiCard
        ...
        <Section
          title="Stripe AU · payment event feed"
          ...
          <SignalCard
            ...
            value={
              billing?.lastConversion
                ? new Date(billing.lastConversion.signedAt).toLocaleString('en-AU')
                : 'No conversions yet'
            }
            hint={
              billing?.lastConversion
                ? `${billing.lastConversion.provider} · A$${(
