      ...
      <div className="space-y-6 max-w-[1400px]">
        <Banner tone="info">
          <span className="text-[13px] flex items-center gap-2">
            ...
            consent-checked + state-clearance-checked before dispatch. Demo data — live wiring lands
            in Phase 1.x.
          </span>
        </Banner>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard
            label="Active rules"
            value={ROUTING_RULES.filter((r) => r.active).length}
            hint="lead routing"
          />
          ...
          action={
            <>
              ...
              <Button
                variant="primary"
                size="sm"
                leftIcon={<Split size={12} />}
                onClick={() => toast.info('New routing rule — rule builder lands in Phase 1.x')}
              >
                New rule
              </Button>
            </>
                ...
                <tr key={r.name}>
                  ...
                  <td className="text-right">
                    <Button
                      ...
                      onClick={() =>
                        toast.info(
                          `${r.active ? 'Pause' : 'Enable'} "${r.name}" — rule toggle wiring lands in Phase 1.x`,
                        )
                      }
                    >
        ...
        <Section
          ...
          action={
            <Button
              ...
              onClick={() => toast.info('New sequence — sequence builder lands in Phase 1.x')}
            >
              New sequence
            </Button>
          }
                ...
                <div className="px-5 py-3.5 border-b border-line2 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4 text-[11px]">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => toast.info(`Edit "${seq.name}" — builder lands in Phase 1.x`)}
                    >
                      Edit
                    </Button>
                  </div>
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3 overflow-x-auto pb-2">
                    {seq.steps.map((step, i) => {
                      ...
                      const colorClass = CHANNEL_COLOR[step.channel];
                      return (
                        <div
                          key={`${seq.name}-${i}-wrap`}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button
                    onClick={() =>
                      toast.info('Apply as A/B test — experiment wiring lands in Phase 1.x')
                    }
                    className="pill pill-info hover:opacity-80 transition cursor-pointer"
                  >
                    Apply (A/B test)
                  </button>
                  <button
                    onClick={() => toast.success('Suggestion dismissed')}
                    className="pill pill-muted hover:opacity-80 transition cursor-pointer"
                  >
                    Dismiss
                  </button>
                </div>
