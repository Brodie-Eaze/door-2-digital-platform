...
const VARIANTS: Array<{ key: VariantKey; title: string; subtitle: string }> = [
  { key: 'login', title: 'Login', subtitle: 'Face ID + Okta SSO' },
  { key: 'map', title: 'Territory map', subtitle: 'Live satellite + knock pins' },
  { key: 'knock-sheet', title: 'Knock sheet', subtitle: 'Disposition wheel + fast form' },
  { key: 'lead-form', title: 'Lead form', subtitle: 'Full intake at the door' },
  { key: 'signature', title: 'Signature', subtitle: 'Pressure-aware signed consent' },
  { key: 'photo', title: 'Door photo', subtitle: 'Capture + auto-encrypt to S3' },
  { key: 'schedule', title: 'Schedule', subtitle: 'Callbacks + shift clock-in' },
  { key: 'inbox', title: 'Inbox', subtitle: 'Manager messages + push' },
  { key: 'pitch-script', title: 'Pitch script', subtitle: 'Branching scripts + objections' },
  { key: 'me', title: 'Me · stats', subtitle: 'Leaderboard + commission preview' },
];

export default function MobilePreviewPage(): JSX.Element {
  const [filter, setFilter] = useState<'all' | VariantKey>('all');
      ...
      <div className="space-y-5 max-w-[1700px]">
        ...
        <div className="card !p-0">
          <div className="flex items-center gap-1.5 px-4 py-3 flex-wrap">
            <span className="text-[11px] text-muted uppercase tracking-wider mr-1">Show</span>
            <button
              onClick={() => setFilter('all')}
              className={`px-2.5 py-1.5 rounded text-[11px] font-medium transition ${
                filter === 'all' ? 'bg-ink text-surface' : 'bg-paper text-muted hover:bg-line2'
              }`}
            >
              All 10 screens
            </button>
            {VARIANTS.map((v) => (
              <button
                ...
                className={`px-2.5 py-1.5 rounded text-[11px] font-medium transition ${
                  filter === v.key ? 'bg-ink text-surface' : 'bg-paper text-muted hover:bg-line2'
                }`}
              >
                {v.title}
              </button>
