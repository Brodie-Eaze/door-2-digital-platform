...
- **Shared-secret law**: the API and every BFF surface (web-operator, web-org) MUST share `JWT_ACCESS_SECRET` and `PII_SEARCH_KEY` (and `AUDIT_CHAIN_SECRET` where both write audit rows). Drift is silent and vicious: mismatched JWT secret makes the BFF reject real API sessions and fall back to demo cookies; mismatched PII_SEARCH_KEY makes BFF-minted users unloginable (emailDigest computed differently at login). Both happened 2026-08-21 in dev. Set them once per environment, from the same source.
- **Gate gotchas**: PATCH routes never take `withIdempotency` (RFC 5789); Next 16 removed `next lint` (web-operator lints via direct eslint, eslint-config-next pinned 14.x while workspace is eslint 8); `_`-prefix doesn't silence no-unused-vars for args unless configured; vitest packages with no tests need `--passWithNoTests`; Semgrep blocks TLS < 1.2 in Terraform; Fastify 5 needs `loggerInstance` for a pino instance (`logger` = options only) and rejects numeric `trustProxy` (use `(addr, hop) => hop === 0`); @fastify/rate-limit errorResponseBuilder bodies need `statusCode` or refusals surface as 500; tests that build their own app never prove `src/index.ts` boots — the CI boot-smoke gate does.
